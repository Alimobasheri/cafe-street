...
export const MenuSectionItems: FC<{
  ...
  if (!has_sub_sections) {
    return (
      <div
        className={`w-full mx-auto flex flex-col p-6 gap-4 backdrop-blur-xl p-2 rounded-lg bg-black/30`}
      >
        {items.map((item) => (
          <div
            key={item.name}
    ...
    return subsections.map((section) => (
      <div
        key={section.name}
        className={`w-full mx-auto mb-2 flex flex-col p-6 gap-4 backdrop-blur-xl p-2 rounded-lg bg-black/30`}
      >
